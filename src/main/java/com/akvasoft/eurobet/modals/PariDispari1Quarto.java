...
@Entity
@Table(name = "T_PARI_DISPARI_1QUARTO")
public class PariDispari1Quarto {
    @Id
    @GeneratedValue(strategy = GenerationType.AUTO)
    @Column(name = "T_PARI_DISPARI_1QUARTO_ID")
    private int id;

    @Column(name = "T_PARI_DISPARI_1QUARTO_PARI")
    private String pari;

    @Column(name = "T_PARI_DISPARI_1QUARTO_DISPARI")
    private String dispari;

    @Column(name = "T_PARI_DISPARI_1QUARTO_SCRAPE_TIME")
    private String scrtime;

    @JsonBackReference
    @ManyToOne(fetch = FetchType.EAGER)
    @JoinColumn(name = "T_PARI_DISPARI_1QUARTO_MATCH")
    private Match match;
